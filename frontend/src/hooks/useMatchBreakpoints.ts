...
import { breakpointMap } from "../contracts/screenBreackpoints";
import { BreakpointChecks, State } from "../interfaces/IScreenBreackpoints";
 ...
 */
const mediaQueries: State<string> = (() => {
  let prevMinWidth = 0;
...
const useMatchBreakpoints = (): BreakpointChecks => {
  const [state, setState] = useState<State<boolean>>(() => getState());
